...
//Удаление товара
$delMorozName = trim($_GET["MR_Base_M_name"]);
if (!empty($delMorozName)) {
    if ($_GET['confirm_del'] == 'Y') {
        //Подключение файла конфигурации
        require_once "login_config.php";

        //Подготавка запроса
        $sql = "DELETE FROM MR_Base_Moroz WHERE MR_Base_M_name = ?";

        if ($stmt = mysqli_prepare($link, $sql)) {
            //Привязка переменных к подготовленному запросу
            mysqli_stmt_bind_param($stmt, "s", $param_m_name);

            //Установка параметров
            $param_m_name = trim($delMorozName);

            //Попытка выполнить подготовленный statement
            if (mysqli_stmt_execute($stmt)) {
                //Запись успешно удалена. Перенаправление на страницу Gallary_Moroz.php
            ...
            <div class="col-md-12">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="get">
                    <div class="alert alert-danger fade in">
                        <input type="hidden" name="MR_Base_M_name" value="<?php echo trim($_GET["MR_Base_M_name"]); ?>"/>
                        ...
                        <p>
                            ...
                            <a href="Gallary_Moroz.php" class="btn btn-default">Нет</a>
                        </p>
                    </div>
                </form>
            </div>
