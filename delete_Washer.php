...
if (!empty($del_id_W)) {
    if ($_GET['confirm_del'] == 'Y') {
        ...
        //Подготавка запроса
        $sql = "DELETE FROM DB_Washer WHERE id_W = ?";

        if ($stmt = mysqli_prepare($link, $sql)) {
            //Привязка переменных к подготовленному запросу
            mysqli_stmt_bind_param($stmt, "s", $param_id_W);

            //Установка параметров
            $param_id_W= trim($del_id_W);
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="page-header">
                    ...
                    echo  "<h1>".$_GET["name_W"]."</h1>";?>
                </div>
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="get">
                    <div class="alert alert-danger fade in">
                        <input type="hidden" name="id_W" value="<?php echo trim($_GET["id_W"]); ?>"/>
                        ...
                        <p>
                            ...
                            <a href="Gallary_A_Machine.php" class="btn btn-default">Нет</a>
                        </p>
                    </div>
                </form>
            </div>
